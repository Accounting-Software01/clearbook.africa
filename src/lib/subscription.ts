...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === 'POST') {
    const { userId, tier, durationInDays, email } = req.body;
    ...
    const payment = await initializePayment(email, priceInNaira);
    ...
    if (reference) {
        ...
        if (paymentDetails.data.status === 'success') {
            const { tier, durationInDays } = paymentDetails.data.metadata;
            await createSubscription(userId as string, tier, durationInDays, true);
